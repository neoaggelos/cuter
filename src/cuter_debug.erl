...
walk_with_anno(T) ->
  ...
  case cerl:type(T) of
    ...
    'fun' ->
      ...
      Taint = not string:is_empty(V),
      get_line(),
      Vars = lists:map(F, cerl:fun_vars(T)),
      get_line(),
      Body = walk_with_anno(cerl:fun_body(T)),
      cerl:update_c_fun(cerl:add_ann([{tainted, Taint}], T), Vars, Body);
    var ->
      L = get_line(),
      V = string:trim(hd(tl(string:split(L, ":")))),
      Taint = not string:is_empty(V),
      cerl:add_ann([{tainted, Taint}], T);
    'apply' ->
      L = get_line(),
      ...
      cerl:update_c_apply(cerl:add_ann([{tainted, Taint}], T), Op, Args);
    literal ->
      get_line(),
      cerl:add_ann([{tainted, false}], T);
    call ->
      L = get_line(),
      ...
      cerl:update_c_call(cerl:add_ann([{tainted, Taint}], T), Mod, Name, Args);
    'case' ->
      L = get_line(),
      ...
      cerl:update_c_case(cerl:add_ann([{tainted, Taint}], T), Arg, Clauses);
    clause ->
      L = get_line(),
      ...
      cerl:update_c_clause(cerl:add_ann([{tainted, Taint}], T), Pats, Guard, Body);
    'let' ->
      L = get_line(),
      ...
      cerl:update_c_let(cerl:add_ann([{tainted, Taint}], T), Vars, Arg, Body);
    letrec ->
      L = get_line(),
      ...
      cerl:update_c_letrec(cerl:add_ann([{tainted, Taint}], T), Defs, Body);
    primop ->
      get_line(),
