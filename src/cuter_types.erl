...
-export([params_of_t_function_det/1, ret_of_t_function/1, atom_of_t_atom_lit/1,
	 ...
	 name_of_t_userdef/1, assocs_of_t_map/1]).
...
-export([t_atom/0, t_atom_lit/1, t_any/0,
	 ...
	 t_range/2, t_pos_inf/0, t_neg_inf/0, t_userdef/1, t_map/0, t_map/1]).
...
-type rep()  :: atom() | bitstring_rep()
              ...
              | record_rep() | remote_rep() | userdef_rep() | map_rep()
...
t_from_form({type, _, map, AssocForms}) ->
  t_map([
    case AssocType of 
      ...
      _ -> throw({unsupported, {A}})
    end || {type, _, AssocType, [From, To]}=A <- AssocForms
  ]);
%% local type
