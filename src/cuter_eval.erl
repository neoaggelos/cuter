...
eval(A, CAs, SAs, CallType, Servers, Fd) ->
  eval(A, CAs, SAs, CallType, Servers, Fd, false).


-spec eval(eval(), [any()], [any()], calltype(), servers(), file:io_device(), cuter_taint_annotation:taint()) -> result().

%% Handle spawns so that the spawned process will be interpreted
%% and not directly executed

%% spawn/{1,2,3,4} & spawn_link/{1,2,3,4}
eval({named, erlang, F}, CAs, SAs, _CallType, Servers, Fd, _) when F =:= spawn; F =:= spawn_link ->
  Arity = length(CAs),
...
%% spawn_monitor/{1,3}
eval({named, erlang, spawn_monitor}, CAs, SAs, _CallType, Servers, Fd, _) ->
  Arity = length(CAs),
  SAs_e = cuter_symbolic:ensure_list(SAs, Arity, Fd),
  Rf = erlang:make_ref(),
  EvalAs =
...
%% spawn_opt/{1,3,4,5}
eval({named, erlang, spawn_opt}, CAs, SAs, _CallType, Servers, Fd, _) ->
  Arity = length(CAs),
  SAs_e = cuter_symbolic:ensure_list(SAs, Arity, Fd),
  Rf = erlang:make_ref(),
  R =
...
eval({named, erlang, '!'}, [_, _] = CAs, SAs, CallType, Servers, Fd, InhT) ->
  eval({named, erlang, send}, CAs, SAs, CallType, Servers, Fd, InhT);
...
%% send_after/3
eval({named, erlang, send_after}, CAs, SAs, _CallType, Servers, Fd, _) ->
  Arity = length(CAs),
  SAs_e = cuter_symbolic:ensure_list(SAs, Arity, Fd),
  case CAs of
    [CTime, CDest_u, CMsg] ->
...
%% send_nosuspend/{2,3}
eval({named, erlang, send_nosuspend}, CAs, SAs, _CallType, Servers, Fd, _) ->
  Arity = length(CAs),
  SAs_e = cuter_symbolic:ensure_list(SAs, Arity, Fd),
  case CAs of
    [CDest_u, CMsg] ->
...
%% throw/1
eval({named, erlang, throw}, CAs, SAs, _CallType, _Servers, Fd, _) ->
  Arity = length(CAs),
  SAs_e = cuter_symbolic:ensure_list(SAs, Arity, Fd),
  case CAs of
    [CThrow] ->
...
%% exit/{1,2}
eval({named, erlang, exit}, CAs, SAs, _CallType, _Servers, Fd, _) ->
  Arity = length(CAs),
  SAs_e = cuter_symbolic:ensure_list(SAs, Arity, Fd),
  case CAs of
    [CExit] ->
...
%% error/{1,2}
eval({named, erlang, error}, CAs, SAs, _CallType, _Servers, Fd, _) ->
  Arity = length(CAs),
  SAs_e = cuter_symbolic:ensure_list(SAs, Arity, Fd),
  case CAs of
    [CError] ->
...
%% raise/3
eval({named, erlang, raise}, CAs, SAs, _CallType, _Servers, Fd, _) ->
  Arity = length(CAs),
  SAs_e = cuter_symbolic:ensure_list(SAs, Arity, Fd),
  case CAs of
...
%% make_fun/3
eval({named, erlang, make_fun}, CAs, SAs, _CallType, Servers, Fd, InhT) ->
  Arity = length(CAs),
  _ = cuter_symbolic:ensure_list(SAs, Arity, Fd),
  case CAs of
    [M, F, A] ->
      make_fun(M, F, A, Servers, Fd, InhT);
    _ ->
      exception(error, {undef, {erlang, make_fun, Arity}})
  end;

%% apply/{2,3}
eval({named, erlang, apply}, CAs, SAs, _CallType, Servers, Fd, InhT) ->
  Arity = length(CAs),
  SAs_e = cuter_symbolic:ensure_list(SAs, Arity, Fd),
  case CAs of
    [Fun, Args] ->
      [SFun, SArgs] = SAs_e,
      %% Constraint: Fun=SFun
      eval({lambda, Fun, SFun}, Args, SArgs, local, Servers, Fd, InhT);
    [M, F, Args] ->
      [_SMod, _SFun, SArgs] = SAs_e,
      %% Constraints: SMod = M, SFun=F
      Call = find_call_type(erlang, M),
      eval({named, M, F}, Args, SArgs, Call, Servers, Fd, InhT);
    _ ->
...
%% Handle an MFA
eval({named, M, F}, CAs_b, SAs_b, CallType, Servers, Fd, InhT) ->
  ...
  case access_mfa_code(MFA, Servers) of
    ...
    {NM, {Def, Exported}} ->
      ...
      eval_expr(Def#c_fun.body, NM, Cenv, Senv, Servers, Fd, InhT)
  end;

%% Handle a Closure
eval({lambda, Closure, ClosureSymb}, CAs, SAs, _CallType, _Servers, Fd, _InhT) ->
  Arity = length(CAs),
  SAs_e = cuter_symbolic:ensure_list(SAs, Arity, Fd),
...
eval({letrec_func, {M, _F, Def, E}}, CAs, SAs, _CallType, Servers, Fd, InhT) ->
  {Cenv, Senv} = E(),
  ...
  eval_expr(Def#c_fun.body, M, NCenv, NSenv, Servers, Fd, InhT).
...
%  end,
  ...
  Fun =
    fun(A) ->
      A_ev = eval_expr(A, M, Cenv, Senv, Servers, Fd, InhT),
      ...
      case get_concrete(A_ev) of
        {?FUNCTION_PREFIX, {F, Arity}} ->
          ...
          create_closure(M, F, Arity, local, Servers, Fd, InhT);
        {letrec_func, {Mod, F, Arity, Def, E}} ->
          %% letrec func
          {Ce, Se} = E(),
          create_closure(Mod, F, Arity, {letrec_func, {Def, Ce, Se}}, Servers, Fd, InhT);
        _ ->
          A_ev
      end
    end,
  ...
  case get_concrete(Op_ev) of % See eval_expr(#c_var{}, ...) output for reference
    {?FUNCTION_PREFIX, {Func, _Arity}} ->
      eval({named, M, Func}, CAs, SAs, local, Servers, Fd, InhT);
    {letrec_func, {Mod, Func, _Arity, Def, E}} ->
      eval({letrec_func, {Mod, Func, Def, E}}, CAs, SAs, local, Servers, Fd, InhT);
    Closure ->
...
eval_expr({c_bitstr, Anno, Val, Size, Unit, Type, Flags}, M, Cenv, Senv, Servers, Fd, InhT) ->
  ...
  Size_ev = eval_expr(Size, M, Cenv, Senv, Servers, Fd, InhT),
  Unit_ev = eval_expr(Unit, M, Cenv, Senv, Servers, Fd, InhT),
  Type_ev = eval_expr(Type, M, Cenv, Senv, Servers, Fd, InhT),
  Flags_ev = eval_expr(Flags, M, Cenv, Senv, Servers, Fd, InhT),
...
eval_expr({c_call, Anno, Mod, Name, Args}, M, Cenv, Senv, Servers, Fd, InhT) ->
  ...
  eval({named, Mod_c, get_concrete(Fv_ev)}, CAs, SAs, find_call_type(M, Mod_c), Servers, Fd, InhT);
  ...
  case erlang:is_function(V) of
	...
	undefined -> mk_result(V, V);
	    ...
	    Mod ->
	      ...
	      make_fun(Mod, Func, Arity, Servers, Fd, InhT)
	  end
      end
  end;
...
eval_expr({c_primop, Anno, Name, Args}, M, Cenv, Senv, Servers, Fd, InhT) ->
  ...
  case PrimOp of
    raise ->
      ...
      eval({named, erlang, Class_c}, [Reason_c], [Reason_s], external, Servers, Fd, InhT);
    match_fail ->
      [Cv] = CAs,
      [Sv] = SAs,
      eval({named, erlang, error}, [{badmatch, Cv}], [{badmatch, Sv}], external, Servers, Fd, InhT);
    _ ->
...
eval_expr({c_receive, Anno, Clauses, Timeout, Action}, M, Cenv, Senv, Servers, Fd, InhT) ->
  ...
  Message = find_message(Mailbox, Clauses, M, Cenv, Senv, Servers, Fd, InhT),
  case Message of
    {Msg, Body, NCenv, NSenv, _Cnt} ->  %% Matched a message already in the mailbox
      receive Msg -> ok end,  %% Just consume the message
      eval_expr(Body, M, NCenv, NSenv, Servers, Fd, InhT);
    false ->  %% No mailbox message matched, thus need to enter a receive loop
      CurrMsgs = length(Mailbox),
      find_message_loop(Clauses, Action, Timeout_c, Timeout_s, Start, CurrMsgs, M, Cenv, Senv, Servers, Fd, InhT)
  end;
...
eval_expr({c_try, Anno, Arg, Vars, Body, Evars, Handler}, M, Cenv, Senv, Servers, Fd, InhT) ->
  ...
  try
    ...
    Class:Reason ->
      ...
      eval_expr(Handler, M, ECe, ESe, Servers, Fd, InhT)
  end;
...
eval_expr(Cerl, _M, _Cenv, _Senv, _Servers, _Fd, _InhT) ->
...
%% --------------------------------------------------------
find_message_loop(Clauses, Action, infinity, STimetout, Start, Msgs, M, Cenv, Senv, Servers, Fd, InhT) ->
  run_message_loop(Clauses, Action, infinity, STimetout, Start, Msgs, M, Cenv, Senv, Servers, Fd, InhT);
find_message_loop(Clauses, Action, CTimeout, STimeout, Start, Msgs, M, Cenv, Senv, Servers, Fd, InhT) ->
  Now = os:timestamp(),
  Passed = timer:now_diff(Now, Start) / 1000,
  case Passed >= CTimeout of
    true ->
      eval_expr(Action, M, Cenv, Senv, Servers, Fd, InhT);
    false ->
      run_message_loop(Clauses, Action, CTimeout, STimeout, Start, Msgs, M, Cenv, Senv, Servers, Fd, InhT)
  end.
...
run_message_loop(Clauses, Action, CTimeout, STimeout, Start, Msgs, M, Cenv, Senv, Servers, Fd, InhT) ->
  ...
  case CurrMsgs > Msgs of
    ...
    true ->
      ...
      Message = find_message(NewMsgs, Clauses, M, Cenv, Senv, Servers, Fd, InhT),
      case Message of
        false ->
          find_message_loop(Clauses, Action, CTimeout, STimeout, Start, CurrMsgs, M, Cenv, Senv, Servers, Fd, InhT);
...
%% --------------------------------------------------------
find_message([], _Clauses, _M, _Cenv, _Senv, _Servers, _Fd, _InhT) ->
  false;

find_message([Msg|Mailbox], Clauses, M, Cenv, Senv, Servers, Fd, InhT) ->
  {LoggerFun, Msg1} = decode_msg(Msg, Fd),
  case find_clause(Clauses, M, 'receive', get_concrete(Msg1), get_symbolic(Msg1), Cenv, Senv, Servers, Fd, InhT) of
    false ->
      find_message(Mailbox, Clauses, M, Cenv, Senv, Servers, Fd, InhT);
...
%% case statement.
%% --------------------------------------------------------
find_clause(Clauses, M, Mode, Cv, Sv, Cenv, Senv, Servers, Fd, InhT) ->
  find_clause(Clauses, M, Mode, Cv, Sv, Cenv, Senv, Servers, Fd, 1, InhT).

find_clause([], _M, _Mode, _Cv, _Sv, _Cenv, _Senv, _Servers, _Fd, _Cnt, _InhT) ->
  false;
find_clause([Cl|Cls], M, Mode, Cv, Sv, Cenv, Senv, Servers, Fd, Cnt, InhT) ->
  Match = match_clause(Cl, M, Mode, Cv, Sv, Cenv, Senv, Servers,  Fd, Cnt, InhT),
  case Match of
    false ->
      find_clause(Cls, M, Mode, Cv, Sv, Cenv, Senv, Servers, Fd, Cnt+1, InhT);
...
%% --------------------------------------------------------
match_clause({c_clause, Anno, Pats, Guard, Body}, M, Mode, Cv, Sv, Cenv, Senv, Servers, Fd, Cnt, InhT) ->
  case is_patlist_compatible(Pats, Cv) of
    ...
    true ->
      ...
      Match = pattern_match_all(Pats, BitInfo, Mode, Cs, Ss_e, Servers, Fd, InhT),
      case Match of
        false -> false;
        {true, {CMs, SMs}} ->
          ...
          Guard_ev = eval_expr(Guard, M, Ce, Se, Servers, Fd, InhT),
...
pattern_match_all(Pats, BitInfo, Mode, Cs, Ss, Servers, Fd, InhT) ->
  pattern_match_all(Pats, BitInfo, Mode, Cs, Ss, [], [], Servers, Fd, InhT).

pattern_match_all([], _BitInfo, _Mode, [], [], CMaps, SMaps, _Servers, _Fd, _InhT) ->
  {true, {CMaps, SMaps}};
pattern_match_all([P|Ps], BitInfo, Mode, [Cv|Cvs], [Sv|Svs], CMaps, SMaps, Servers, Fd, InhT) ->
  Match = pattern_match(P, BitInfo, Mode, Cv, Sv, CMaps, SMaps, Servers, Fd, InhT),
  case Match of
    false -> false;
    {true, {CMs, SMs}} ->
      pattern_match_all(Ps, BitInfo, Mode, Cvs, Svs, CMs, SMs, Servers, Fd, InhT)
  end.
...
pattern_match({c_literal, Anno, LitVal}, _Bitinfo, _Mode, Cv, Sv, CMaps, SMaps, Servers, Fd, _InhT) ->
  Tags = cuter_cerl:get_tags(Anno),
  case LitVal =:= Cv of
    true ->
...
pattern_match({c_var, _Anno, Name}, _BitInfo, _Mode, Cv, Sv, CMaps, SMaps, _Servers, _Fd, _InhT) ->
  CMs = [{Name, Cv}|CMaps],
  SMs = [{Name, Sv}|SMaps],
  {true, {CMs, SMs}};

%% Tuple pattern
pattern_match({c_tuple, Anno, Es}, BitInfo, Mode, Cv, Sv, CMaps, SMaps, Servers, Fd, InhT) when is_tuple(Cv) ->
  Ne = length(Es),
  Tags = cuter_cerl:get_tags(Anno),
  case tuple_size(Cv) =:= Ne of
    true ->
      ...
      pattern_match_all(Es, BitInfo, Mode, Cv_l, Sv_l, CMaps, SMaps, Servers, Fd, InhT);
    false ->
...
pattern_match({c_tuple, Anno, Es}, _BitInfo, _Mode, _Cv, Sv, _CMaps, _SMaps, Servers, Fd, _InhT) ->
  Ne = length(Es),
  Tags = cuter_cerl:get_tags(Anno),
...
pattern_match({c_cons, Anno, _Hd, _Tl}, _BitInfo, _Mode, [], Sv, _CMaps, _SMaps, Servers, Fd, _InhT) ->
  Tags = cuter_cerl:get_tags(Anno),
  %% CONSTRAINT: Sv is an empty list
  visit_tag(Servers#svs.code, Tags#tags.next),
  cuter_log:log_list(Fd, empty, Sv, Tags#tags.this),
  false;
pattern_match({c_cons, Anno, Hd, Tl}, BitInfo, Mode, [Cv|Cvs], Sv, CMaps, SMaps, Servers, Fd, InhT) ->
  Tags = cuter_cerl:get_tags(Anno),
  ...
  case pattern_match(Hd, BitInfo, Mode, Cv, Sv_h, CMaps, SMaps, Servers, Fd, InhT) of
    false -> false;
    {true, {CMs, SMs}} -> pattern_match(Tl, BitInfo, Mode, Cvs, Sv_t, CMs, SMs, Servers, Fd, InhT)
  end;
pattern_match({c_cons, Anno, _Hd, _Tl}, _BitInfo, _Mode, _Cv, Sv, _CMaps, _SMaps, Servers, Fd, _InhT) ->
  Tags = cuter_cerl:get_tags(Anno),
...
%% Alias pattern
pattern_match({c_alias, _Anno, Var, Pat}, BitInfo, Mode, Cv, Sv, CMaps, SMaps, Servers, Fd, InhT) ->
  Match = pattern_match(Pat, BitInfo, Mode, Cv, Sv, CMaps, SMaps, Servers, Fd, InhT),
  case Match of
    false -> false;
    {true, {CMs, SMs}} ->
...
%% Binary pattern
pattern_match({c_binary, Anno, Segments}, BitInfo, Mode, Cv, Sv, CMaps, SMaps, Servers, Fd, InhT) ->
  bit_pattern_match(Anno, Segments, BitInfo, Mode, Cv, Sv, CMaps, SMaps, Servers, Fd, InhT).
...
bit_pattern_match(BinAnno, [], _BitInfo, _Mode, <<>>, Sv, CMaps, SMaps, Servers, Fd, _InhT) ->
  %% CONSTRAINT: Sv =:= <<>>
  Tags = cuter_cerl:get_tags(BinAnno),
  visit_tag(Servers#svs.code, Tags#tags.this),
  cuter_log:log_equal(Fd, true, <<>>, Sv, Tags#tags.next),
  {true, {CMaps, SMaps}};
bit_pattern_match(BinAnno, [], _BitInfo, _Mode, _Cv, Sv, _CMaps, _SMaps, Servers, Fd, _InhT) ->
  %% CONSTRAINT: Sv =/= <<>>
  Tags = cuter_cerl:get_tags(BinAnno),
  visit_tag(Servers#svs.code, Tags#tags.next),
...
bit_pattern_match(BinAnno, [{c_bitstr, Anno, {c_literal, _, LVal}, Sz, Unit, Tp, Fgs}|Bs], {M, Cenv, Senv} = Bnfo, Mode, Cv, Sv, CMaps, SMaps, Svs, Fd, InhT) ->
  Size_ev = eval_expr(Sz, M, Cenv, Senv, Svs, Fd, InhT),
  Unit_ev = eval_expr(Unit, M, Cenv, Senv, Svs, Fd, InhT),
  Type_ev = eval_expr(Tp, M, Cenv, Senv, Svs, Fd, InhT),
  Flags_ev = eval_expr(Fgs, M, Cenv, Senv, Svs, Fd, InhT),
  Size_c = get_concrete(Size_ev),
  Size_s = get_symbolic(Size_ev),
  ...
  try cuter_binlib:match_bitstring_const(LVal, Size_c, get_concrete(Unit_ev), get_concrete(Type_ev), get_concrete(Flags_ev), Cv) of
    Rest_c ->
      ...
      bit_pattern_match(BinAnno, Bs, Bnfo, Mode,  Rest_c, Rest_s, CMaps, SMaps, Svs, Fd, InhT)
  catch
    error:_e ->
...
bit_pattern_match(BinAnno, [{c_bitstr, Anno, {c_var, _, VarName}, Sz, Unit, Tp, Fgs}|Bs], {M, Cenv, Senv}, Mode, Cv, Sv, CMaps, SMaps, Svs, Fd, InhT) ->
  Size_ev = eval_expr(Sz, M, Cenv, Senv, Svs, Fd, InhT),
  Unit_ev = eval_expr(Unit, M, Cenv, Senv, Svs, Fd, InhT),
  Type_ev = eval_expr(Tp, M, Cenv, Senv, Svs, Fd, InhT),
  Flags_ev = eval_expr(Fgs, M, Cenv, Senv, Svs, Fd, InhT),
  ...
  try cuter_binlib:match_bitstring_var(Size_c, get_concrete(Unit_ev), get_concrete(Type_ev), get_concrete(Flags_ev), Cv) of
    {X_c, Rest_c} ->
      ...
      bit_pattern_match(BinAnno, Bs, {M, NCenv, NSenv}, Mode, Rest_c, Rest_s, CMs, SMs, Svs, Fd, InhT)
  catch
    error:_E ->
      visit_tag(Svs#svs.code, Tags#tags.next),
...
create_closure(M, F, Arity, local, Servers, Fd, InhT) ->
  ...
  make_fun(Def#c_fun.vars, Def#c_fun.body, NM, Arity, Cenv, Senv, Servers, Fd, InhT);

%% Create a Closure when the MFA is a function bound in a letrec
create_closure(M, _F, Arity, {letrec_func, {Def, Cenv, Senv}}, Servers, Fd, InhT) ->
  make_fun(Def#c_fun.vars, Def#c_fun.body, M, Arity, Cenv, Senv, Servers, Fd, InhT).
...
make_fun(Vars, Body, Mod, Arity, Cenv, Senv, Servers, Fd, InhT) ->
  Creator = self(),
  LambdaS = cuter_symbolic:fresh_lambda(Arity, Fd),
  add_to_created_closure(LambdaS),
  LambdaC =
    case Arity of
      0 ->
        fun() ->
          make_fun_h1(Mod, [], Servers, Vars, Body, Cenv, Senv, Creator, LambdaS, Fd, InhT)
        end;
      1 ->
        fun(A) ->
          Args = [A],
          make_fun_h1(Mod, Args, Servers, Vars, Body, Cenv, Senv, Creator, LambdaS, Fd, InhT)
        end;
      2 ->
        fun(A, B) ->
          Args = [A, B],
...
make_fun_h1(Mod, Args, Servers, Vars, Body, Cenv, Senv, Creator, LambdaS, FileDescr, InhT) ->
  ...
  Ret = eval_expr(Body, Mod, Ce, Se, NSvs, Fd, InhT),
...
make_fun(Mod, Func, Arity, Servers, Fd, InhT) ->
  ...
  LambdaC =
    case Arity of
      0 ->
        fun() ->
          make_fun_h(Mod, Func, [], Servers, Creator, LambdaS, Fd, InhT)
        end;
      1 ->
        fun(A) ->
          Args = [A],
          make_fun_h(Mod, Func, Args, Servers, Creator, LambdaS, Fd, InhT)
        end;
      2 ->
        fun(A, B) ->
          Args = [A, B],
...
make_fun_h(Mod, Func, Args, Servers, Creator, LambdaS, FileDescr, InhT) ->
  {CAs, SAs} = unzip_args(Args), %% If Args =:= [] then unzip_args([]) will return {[], []}
  NSvs = validate_servers(Servers),
  Fd = validate_file_descriptor(NSvs#svs.monitor, Creator, FileDescr),
  Ret = eval({named, Mod, Func}, CAs, SAs, external, NSvs, Fd, InhT),
  cuter_log:log_evaluated_closure(Fd, LambdaS, SAs, get_symbolic(Ret)),
  Ret.
